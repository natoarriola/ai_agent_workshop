...
def story():
    ...
    out += [table([
        ["", "Status", "Detail"],
        ["Golden tests", tag("17 / 20", WARN), "vs. real bedtools; the 3 remaining are subtract"],
        ["Unit tests", tag("102 passing", GOOD), "stdlib unittest, 4 ms, no bedtools needed"],
        ...
        ["Merged to main", tag("nothing yet", WARN), "6 pull requests open in a stack"],
    ], [30 * mm, 27 * mm, W - 57 * mm])]
    ...
    out += [table([
        ...
        ["subtract", "-a, -b", tag("in progress - 0/3 golden", WARN)],
        ["closest", "-", tag("out of scope for v1", MUTED)],
    ], [28 * mm, 45 * mm, W - 73 * mm])]

    out += [p(
        "Scope was cut deliberately. <font face='Courier'>closest</font> was dropped from v1 when "
        "the spec was adopted, on the principle that four finished subcommands beat seven "
        "half-built ones."
    )]
    ...
    out += [p(
        ...
        "<i>different in all three cases</i>:"
    )]
    out += [table([
    ...
    out += [KeepTogether([
        ...
        table([
            ...
            ["<b>Merge the PR stack</b>",
             "Six PRs chain off each other (#4, #5 → #6 → #8 → #10 → #12). main is still at the upstream commit."],
            ["<b>subtract</b>", "The last v1 subcommand; 3 golden cases."],
        ], [46 * mm, W - 46 * mm]),
    ])]
