...
**My fork is `natoarriola/ai_agent_workshop`.**

Anything that **creates** — `gh issue create`, `gh pr create` — passes
`--repo natoarriola/ai_agent_workshop`. Never write to
...
`mytools` is a small reimplementation of a subset of bedtools. v1 is `sort`, `merge`,
`intersect`, `subtract` — `closest` is explicitly out of scope (see `SPEC.md` §1).
Real `bedtools` is installed and is the oracle — if our output differs from it on the
same input, we are wrong.

## Language

**`mytools` is written in Python 3.** Every subcommand and every test. Don't
